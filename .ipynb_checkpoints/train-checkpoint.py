...
def parse():
    ...
    parser.add_argument("--foldN", type=int, default=4, help = "0|1|2|3|4|5")    
...
def train(trainDataLoader, validDataLoader, net, lossFn, optimizer, config) :
    ...
    for e in range(config.epochN):
        ...
        if maxAuc < auc :
            print(f"({config.foldN}) (saving) the current auc {auc} is bigger than the previous auc {maxAuc}")    
            directory = f"{config.ckptPath}/{config.pretrain}/{config.mode}/{config.mil}/{config.diagnosis}/{config.foldN}"
            os.makedirs(directory, exist_ok=True)
            torch.save(net.state_dict(), f"{directory}/weights.pt")

            maxAuc = auc
        else :
            print(f"({config.foldN}) (aborting) the current auc {auc} is smaller than the previous auc {maxAuc}")
