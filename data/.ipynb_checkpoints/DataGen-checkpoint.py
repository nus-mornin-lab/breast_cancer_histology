...
from sklearn.model_selection import StratifiedKFold as SFold
...
class DataGen(Dataset):
    ...
    def _parse(self):
        ...
        getCaseId = lambda x : re.findall("(S\d+-\d+)_.*", x.split("/")[-1])[0]

        files = glob.glob(f"{self.dataDir}/*.pt")
        
        dataCsv = pd.DataFrame({"PATHOLOGY_NO" : compose(list, map(getCaseId))(files),
                                "dataPath"     : files})

        labelCsv = pd.read_csv(self.labelPath)
        
        csv = pd.merge(dataCsv, labelCsv, how = "inner", on = "PATHOLOGY_NO")

        datapoints = csv[["dataPath", self.diag, "PATHOLOGY_NO"]].values.tolist()
        
        return makeFold(*self.kFold, self.train)(datapoints)
    
#########################################################################################################
@curry
def makeFold(kFold, k, train, datapoints):

    random.seed(0); random.shuffle(datapoints)
    
    sFold = SFold(n_splits = kFold)
    
    Y = compose(list, map(second))(datapoints)
    
    trainIdx, testIdx = list(sFold.split(Y, Y))[k]
    
    if train : return [datapoints[idx] for idx in trainIdx]
    else     : return [datapoints[idx] for idx in testIdx]
